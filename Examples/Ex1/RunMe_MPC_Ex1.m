...
Model_data.A_scaled=[0;0;50];
%B(i,i)=(max(state(i))-min(state(i)))/2
Model_data.B_scaled=diag([l;2;50]);
%C(i)=(max(control(i))+min(control(i)))/2
Model_data.C_scaled=[0].';
%D(i,i)=(max(control(i))-min(control(i)))/2
Model_data.D_scaled=diag([20]);

Model_data.initial_state_Real=[((0)-Model_data.A_scaled(1))/Model_data.B_scaled(1,1) ((1)-Model_data.A_scaled(2))/Model_data.B_scaled(2,2) -1].';
Model_data.initial_state_hat=[((0)-Model_data.A_scaled(1))/Model_data.B_scaled(1,1) ((1)-Model_data.A_scaled(2))/Model_data.B_scaled(2,2) -1].';
...
%% MPC parameters
PMC_params.Ts=0.2;      % Control Sampling Time (s)
PMC_params.m=2;          % Control Horizon
PMC_params.p=3;          % Prediction Horizon
...
for time_index=1:numel(time_finemesh)
    u_star_val(time_index)=u_star_func(time_finemesh(time_index),l);
end

params.l=l;
odefun=@(t,x) dynamic(t,x,params);

[t_star,x_star] = ode45(odefun,time_finemesh,[0 1].');
...
figure()
plot(Time_Out,intermediateData.scaled_control_matrix,'-','linewidth',1);
hold on;
for iter=1:intermediateData.num_total_iterations
    plot(intermediateData.time_cell_iters_h{iter},intermediateData.scaled_control_one_P_in_h{iter},'--','linewidth',3);
end

figure()
plot(Time_Out,intermediateData.scaled_states_matrix_hat(:,1),'-','linewidth',1);
hold on;
for iter=1:intermediateData.num_total_iterations
    plot(intermediateData.time_cell_iters_h{iter},intermediateData.scaled_states_hat_iters_one_P_in_h{iter}(:,1),'--','linewidth',3);
end

figure()
plot(Time_Out,intermediateData.scaled_states_matrix_hat(:,2),'-','linewidth',1);
hold on;
for iter=1:intermediateData.num_total_iterations
    plot(intermediateData.time_cell_iters_h{iter},intermediateData.scaled_states_hat_iters_one_P_in_h{iter}(:,2),'--','linewidth',3);
end
